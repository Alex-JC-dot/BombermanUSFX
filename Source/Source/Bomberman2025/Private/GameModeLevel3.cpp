...
void AGameModeLevel3::BeginPlay()
{
    ...
    {
        //3
        for (int32 fila = 0; fila < MapaBloques.Num(); ++fila)
        {
            for (int32 columna = 0; columna < MapaBloques[fila].Num(); ++columna)
            {
                ...
                FVector Ubicacion_de_Bloques = Bloque_Inicial + FVector(fila * Espaciado, columna * Espaciado, 0);
                 Spawnearbloques(Ubicacion_de_Bloques, valor);
                  //  SpawnearBloqueRandom(Ubicacion_de_Bloques,valor);
    ...
    EliminarBLoque();
    UE_LOG(LogTemp, Warning, TEXT("Total de Bloque: %d"), TodosLosBloques.Num());
    BloqueAcero();
}
